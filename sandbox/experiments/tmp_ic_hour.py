...
scenarios = [
    ("Strangle 08:00",       {"entry_hour": 8}),
    ("Strangle 15:00",       {"entry_hour": 15}),
    ("SL6% 08:00",           {"intraday_sl_pct": 0.06, "entry_hour": 8}),
    ("SL6% 15:00",           {"intraday_sl_pct": 0.06, "entry_hour": 15}),
    ("IC8% 08:00",           {"hedge_otm_pct": 0.08, "entry_hour": 8}),
    ("IC8% 15:00",           {"hedge_otm_pct": 0.08, "entry_hour": 15}),
    ("IC8%+SL6% 08:00",     {"hedge_otm_pct": 0.08, "intraday_sl_pct": 0.06, "entry_hour": 8}),
    ("IC8%+SL6% 15:00",     {"hedge_otm_pct": 0.08, "intraday_sl_pct": 0.06, "entry_hour": 15}),
]
...
for i, (label, overrides) in enumerate(scenarios):
    try:
        ...
        print(f"[{i+1}/{len(scenarios)}] {label:20s} Final={final_eq:>8.1f}  DD={max_dd:>6.1f}%",
...
lines = []
lines.append("")
lines.append("1% OTM 0DTE �?Entry Hour x Strategy Comparison")
lines.append("=" * 100)
lines.append(f"  {'Strategy':<20s} {'Final ETH':>10s} {'MaxDD%':>8s} {'Sharpe':>8s} {'WR%':>6s} {'Trades':>7s}")
lines.append("=" * 100)
for r in results:
    lines.append(
        f"  {r['label']:<20s} {r['final']:>10.1f} {r['max_dd']:>7.1f}% {r['sharpe']:>8.2f} "
        f"{r['wr']:>5.1f}% {r['trades']:>7.0f}"
    )
lines.append("=" * 100)
text = "\n".join(lines)
