...
from exchanges import OptionQuote, fetch_all_quotes
...
def load_quotes(
    ...
    # Convert to dicts for st.cache_data compatibility
    rows = []
    for q in quotes:
        rows.append({
            "exchange": q.exchange,
...
# Spot prices per exchange
spot_data = (
    filtered.groupby(["exchange", "underlying"])["underlying_price"]
    .first()
    .unstack()
)
...
styled = (
    ...
    .style
    .map(highlight_apr, subset=["APR(%)", "净APR(%)"])
    .format({
        "行权价": "${:,.0f}",
        "DTE(天)": "{:.1f}",
        "Bid($)": "${:,.2f}",
...
""")
    ...
    with col2:
        ...
        # Use extrinsic (time) value only, not total premium
...
""")
    ...
    with col3:
        ...
        # Price range: ±20% around strike
        _lo = _strike * 0.80
        _hi = _strike * 1.20
        _prices = np.linspace(_lo, _hi, 200)

        if _is_call:
            # Covered Call = hold spot + short call
            # PnL = (S_T - S_0) + premium - max(0, S_T - K)
            # where S_0 = current spot
            _pnl = (_prices - _spot) + _premium - np.maximum(0, _prices - _strike)
        else:
            # Cash-Secured Put = short put + cash
            # PnL = premium - max(0, K - S_T)
        ...
        # Strike line
        fig.add_vline(
            x=_strike, line_dash="dot", line_color="red", line_width=1,
            annotation_text=f"K={_strike:,.0f}",
        ...
        # Current price line
        fig.add_vline(
            x=_spot, line_dash="dot", line_color="green", line_width=1,
            annotation_text=f"现价={_spot:,.0f}",
        ...
        # Breakeven point
        if _is_call:
            _be = _spot - _premium  # CC breakeven
        else:
            _be = _strike - _premium  # SP breakeven
        fig.add_vline(
            x=_be, line_dash="dashdot", line_color="orange", line_width=1,
            annotation_text=f"盈亏平衡={_be:,.0f}",
        ...
        fig.add_annotation(
            x=_strike if not _is_call else _strike * 1.05,
            y=_max_profit,
            text=f"最大盈利: ${_max_profit:,.0f}",
        ...
        fig.update_layout(
            ...
            showlegend=False,
            title=dict(
                text=f"{'Covered Call' if _is_call else 'Short Put'} 到期损益 (USD)",
                font=dict(size=13),
            ),
        )
        ...
        # Deribit coin-margined: premium is in BTC/ETH
        _premium_coin = _premium / _spot  # convert USD premium to coin

        if _is_call:
            # CC coin-margined: hold 1 coin, short call
            # At expiry: if S_T <= K, keep coin + premium_coin → PnL = premium_coin
            # if S_T > K, deliver coin at K, get K/S_T coin back + premium_coin
            # PnL(coin) = premium_coin - max(0, S_T - K) / S_T
            _pnl_coin = _premium_coin - np.maximum(0, _prices - _strike) / _prices
        else:
            # SP coin-margined: short put, margin in coin
            # PnL(coin) = premium_coin - max(0, K - S_T) / S_T
        ...
        fig2.add_annotation(
            x=_strike if not _is_call else _strike * 1.05,
            y=_max_coin,
            text=f"最大盈利: {_max_coin:.6f} {best['underlying']}",
            showarrow=True, arrowhead=2,
            font=dict(size=10, color="green"),
        )
        ...
        fig2.update_layout(
            ...
            showlegend=False,
            title=dict(
                text=f"{'Covered Call' if _is_call else 'Short Put'} 到期损益 (币本位)",
                font=dict(size=13),
            ),
        )
...
# ---------------------------------------------------------------------------
# Detailed comparison by exchange
# ---------------------------------------------------------------------------

st.markdown("---")
st.subheader("📋 各交易所对比 (同档位)")

# Group by underlying + strike + expiry, pivot by exchange
if len(sel_exchanges) > 1:
    compare_df = filtered.copy()
    compare_df["key"] = (
        compare_df["underlying"] + " "
        + compare_df["strategy"] + " "
        + compare_df["strike"].astype(str) + " "
        + compare_df["expiry"]
    )

    # Find keys present in multiple exchanges
    key_exchanges = compare_df.groupby("key")["exchange"].nunique()
    multi_keys = key_exchanges[key_exchanges > 1].index

    if len(multi_keys) > 0:
        multi = compare_df[compare_df["key"].isin(multi_keys)].copy()
        multi = multi.sort_values("apr", ascending=False)
...
with st.expander(f"📄 完整数据 ({len(filtered)} 条)", expanded=False):
    ...
    st.dataframe(
        filtered[full_cols].rename(columns=col_rename),
        use_container_width=True,
        height=600,
    )
