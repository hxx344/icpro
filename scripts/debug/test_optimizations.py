...
Usage: python scripts/debug/test_optimizations.py
"""
...
def main():
    ...
    print(f"  鏈€楂樻敹鐩?    {best_ret['label']}  ({best_ret['total_return']:.2%})")
    print(f"  鏈€楂?Sharpe: {best_sharpe['label']}  ({best_sharpe['sharpe_hedged']:.2f})")
    print(f"  鏈€灏忓洖鎾?    {best_dd['label']}  ({best_dd['max_dd_hedged']:.2%})")
    print(f"  鏈€楂?PF:     {best_pf['label']}  ({best_pf['profit_factor']:.2f})")
    print(f"{'='*80}")
