"""Grid search for weekly BTC Monday-entry Friday-expiry short strangles.

Sweeps OTM widths and per-leg stop-loss levels, prints a compact table,
and saves full results to reports/weekly_short_monfri_btc_otm_sl_grid.json.
"""
...
BASE_CONFIGS = {
    0.03: "configs/backtest/weekly_short_monfri_btc_otm3.yaml",
    0.05: "configs/backtest/weekly_short_monfri_btc_otm5.yaml",
    0.07: "configs/backtest/weekly_short_monfri_btc_otm7.yaml",
    0.10: "configs/backtest/weekly_short_monfri_btc_otm10.yaml",
}


def run_case(otm_pct: float, stop_loss_pct: int) -> dict:
    cfg = Config.from_yaml(BASE_CONFIGS[otm_pct])
    ...
    cfg.backtest.name = f"MonFri Short Strangle OTM{otm_pct*100:.0f}% {label_sl}"
...
rows: list[dict] = []
total = len(OTM_PCTS) * len(STOP_LOSSES)

print("BTC Weekly Short Strangle (Mon entry, Fri expiry) OTM x StopLoss Grid")
print(f"{'#':>2s} {'OTM':>5s} {'SL':>6s} {'Ret':>8s} {'MaxDD':>8s} {'Sharpe':>7s} {'WR':>6s} {'PF':>6s} {'Trades':>6s} {'Fees':>10s} {'Time':>6s}")
print("=" * 110)

for idx, (otm_pct, stop_loss_pct) in enumerate(itertools.product(OTM_PCTS, STOP_LOSSES), start=1):
    row = run_case(otm_pct, stop_loss_pct)
    rows.append(row)
    sl_label = "NoSL" if stop_loss_pct >= 9999 else str(stop_loss_pct)
    print(
        f"{idx:>2d} {otm_pct*100:>4.0f}% {sl_label:>6s} "
...
if valid:
    ...
    best_dd = max(valid, key=lambda r: r["max_drawdown"])  # less negative is better

    print("Best by return:")
    print(
        f"  OTM {best_return['otm_pct']*100:.0f}% / SL "
    ...
    print(
        f"  OTM {best_sharpe['otm_pct']*100:.0f}% / SL "
    ...
    print(
        f"  OTM {best_dd['otm_pct']*100:.0f}% / SL "
...
report_path = Path("reports/weekly_short_monfri_btc_otm_sl_grid.json")
