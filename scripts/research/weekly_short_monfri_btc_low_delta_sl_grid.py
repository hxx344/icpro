"""Grid search for low-delta weekly BTC Monday-entry Friday-expiry short strangles.

Sweeps lower entry delta targets and per-leg stop-loss levels, prints a compact
summary, and saves full results to reports/weekly_short_monfri_btc_low_delta_sl_grid.json.
"""
...
BASE_CONFIG = "configs/backtest/weekly_short_monfri_btc_delta10.yaml"
...
def run_case(target_delta: float, stop_loss_pct: int) -> dict:
    ...
    cfg.backtest.name = f"MonFri Short Strangle LowΔ{target_delta:.2f} {label_sl}"
...
print("BTC Weekly Short Strangle (Mon entry, Fri expiry) Low Delta x StopLoss Grid")
...
report_path = Path("reports/weekly_short_monfri_btc_low_delta_sl_grid.json")
