"""测试不同入场时间（entry_hour）对裸卖 Call 收益的影响�?

Usage: python scripts/test_entry_hour.py
"""
...
# 测试的入场小�?(UTC)
# Deribit 0-DTE 合约�?08:00 UTC 到期/上新
# 8=到期即刻卖出, 9=延迟1h, ... 16=延迟8h, 0=提前8h(前一�?
...
def main():
    ...
    print(f"\n\n{'='*120}")
    print(f"  入场时间敏感性分�? (ETH Naked Call 0-DTE, 90 days, Baseline = 08:00 UTC)")
    print(f"{'='*120}")
    ...
    for r in results:
        ...
        marker = " ◀ baseline" if r["entry_hour"] == 8 else ""
        line = (
    ...
    # 关键时段分析
    print(f"\n{'='*80}")
    print("  关键时段对比 (vs 08:00 baseline)")
    print(f"{'='*80}")
    ...
    for h in key_hours:
        ...
        delta_usd = r["hedged_usd"] - baseline["hedged_usd"]
        delay = (h - 8) % 24
        print(f"  {h:02d}:00 UTC (延迟{delay:>2d}h): 收益 {r['total_return']:>7.2%} "
    ...
    print(f"\n  最佳入场时�?(收益): {best['entry_hour']:02d}:00 UTC �?{best['total_return']:.2%}")
    print(f"  最差入场时�?(收益): {worst['entry_hour']:02d}:00 UTC �?{worst['total_return']:.2%}")
    print(f"  最�?PF:            {best_pf['entry_hour']:02d}:00 UTC �?PF {best_pf['profit_factor']:.2f}")
    print(f"  最小回�?            {least_dd['entry_hour']:02d}:00 UTC �?{least_dd['max_dd_hedged']:.2%}")
    print(f"{'='*80}")
